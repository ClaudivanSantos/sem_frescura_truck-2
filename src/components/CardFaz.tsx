...
import { fetchPedidosPendenteFazer } from "../services/getPendenteFazer";
...
export default function CardFaz() {
    ...
    const subscribeToOrders = async () => {
      const subscription = supabase
        ...
        .on(
          ...
          async (payload) => {
            console.log("Pedido alterado:", payload.new);
            const novosPedidos = await fetchPedidosPendenteFazer();
  ...
  function handleOrderEdit(id: number, comanda_mesa: string) {
    router.push({
      pathname: `/editar_pedido`,
          ...
          <View key={pedido.id}>
            ...
            {isAdm && (
              <View className="flex-row gap-3 mt-4 justify-center">
                ...
                >
                  <View className="bg-slate-700 items-center flex-row justify-center rounded-lg p-5">
                    <Ionicons
                ...
                <TouchableOpacity
                  onPress={() =>
                    handleStatusUpdate(pedido.id, "pendente_pagamento")
                  }
                >
                  <View className="bg-black items-center flex-row justify-center rounded-lg p-5">
